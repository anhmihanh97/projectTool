...
export class TestFreedomComponent
  ...
  ngOnInit() {
    ...
    this.displayTime = '';
    this.examService.getExam(this.idExam).subscribe(res2 => {
      this.tenBaiThi=res2['name'];
      this.chuDeBaiThi=res2['subject']['name'];
    });
  ...
  scoreExam(): void {
    ...
    this.examService.getFreeTestResult(formData).subscribe(resp => {
      ...
      sessionStorage.setItem("listQuestion", JSON.stringify(resp['listQuestion']));
      sessionStorage.setItem("listExamAnswer", JSON.stringify(resp['listExamAnswer']));
      ...
      this.router.navigate(['/hometotal/testresult', { param1: this.idExam,param2: this.examUserID }]);
    });
  }
