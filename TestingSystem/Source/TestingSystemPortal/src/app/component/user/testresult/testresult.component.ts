...
export class TestresultComponent implements OnInit {
  ...
  constructor(
    ...
    this.activeRoute.params.forEach(urlParams => {
      ...
      this.examUserID = urlParams['param2'];
  ...
  ngOnInit() {
    ...
    if (token != null) {
      ...
      this.examService
        .getExamReSultByExamAndId(this.examUserID, this.examId)
        .pipe(
          tap(output => {
            ...
            this.lenAnswered = output['lenAnswered'];
            const av = output['time'].split(':');
            const tavt = av[0] * 60 + av[1] * 1;
            this.timeperques = Math.round(
              ((tavt * 1) / output['exam']['question_num']) * 1
            );
            // this.question_not_answered = this.question_num - ((this.corect) + (this.incorect));

            this.question_not_answered = this.question_num -  this.lenAnswered;
            if(this.corect == 0 && this.lenAnswered != 0){
              this.incorect = output['correct_num'] + this.lenAnswered;
            }else{
              this.incorect = output['exam']['question_num'] - output['correct_num'] - this.question_not_answered;
            }
                 }),
          tap(output => {
            this.examService
    ...
    this.examService.getListQuestionExam(this.examId).subscribe(res => {
      ...
      if (this.len > 0) {
        ...
        this.examService
          .getListQuestionResult(this.examUserID)
          .subscribe(res1 => {
            if (res1.length > 0) {
